...
var callback = function(challengeData){

	// challengeData = JSON.parse(data);
	localStorage.cache = challengeData;
...
var challengeData = function () {
	...
	$.ajax({
		type:'GET',
		dataType: 'jsonp',
		url: 'http://testchallengehunt.appspot.com/v1/all?callback=jsonp',
...
function main (){
	...
	$('.middleNav li').click(function(){

		$('.middleNav li').removeClass('active');
		$('.middleNav li').removeClass('unactive');
		$(this).addClass('active');
		$(this).siblings().addClass('unactive');
	});

	$('.filterClass p').click(function(){

		var clickedElement = $(this).attr("id");
		var clickedCategory = $('.activeCards').attr("id");
		console.log(clickedElement,clickedCategory);
		...
		if(clickedElement == "All"){
			...
			$("#"+clickedCategory+"_Active").css("display","none");
			$("#"+clickedCategory+"_Upcoming").css("display","block");
		}
	});
